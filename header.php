...
$error_message = '';
$success_message = '';
$statement = $pdo->prepare("SELECT * FROM Maleda_News_settings WHERE id=?");
...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_subscriber WHERE subs_active=0");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach($result as $row)
{
	...
	if($res > 1440)
	{
		$statement1 = $pdo->prepare("DELETE FROM Maleda_News_subscriber WHERE subs_id=?");
...
<head>
	...
	{
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_category WHERE category_slug=?");
		$statement->execute(array($_REQUEST['slug']));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
		foreach ($result as $row)
		{
...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_comment WHERE id=1");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) 
{
					...
					<div class="col-md-9 col-sm-9 nav-wrapper">
						...
						<nav>
							<ul class="sf-menu" id="menu">
								...
								$statement = $pdo->prepare("SELECT * FROM Maleda_News_menu ORDER BY menu_order ASC");
								$statement->execute();
								$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
								foreach ($result as $row) 
								{
									...
									$statement1 = $pdo->prepare("SELECT * FROM Maleda_News_menu WHERE menu_parent=?");
