...
else
{
	...
	$statement = $pdo->prepare("SELECT * FROM Maleda_News_subscriber WHERE subs_email=? AND subs_hash=?");
...
$statement = $pdo->prepare("UPDATE Maleda_News_subscriber SET subs_hash=?, subs_active=? WHERE subs_email=?");
