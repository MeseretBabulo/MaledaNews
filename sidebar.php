...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_advertisement WHERE adv_id=2");
$statement->execute();
...
<div class="row">
	<div class="col-md-12">
		<div class="heading">
			<h2 class="sidebar-heading">ፈልግ</h2>
		</div>
	</div>
</div>
<div class="row">
...
<div class="row">
	<div class="col-md-12">
		<div class="heading">
			<h2 class="sidebar-heading">ማህበራዊ ሚዲያ</h2>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-12">
		<div class="social-sidebar">
			<ul>
				...
				$statement = $pdo->prepare("SELECT * FROM Maleda_News_social");
				$statement->execute();
				$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
				foreach ($result as $row) 
				{
...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_settings WHERE id=1");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) 
{
...
<div class="row">
	<div class="col-md-12">
		<div class="heading">
			<h2 class="sidebar-heading">የቅርብ ጊዜ ዜናዎች</h2>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-sm-12 col-md-12">
		...
		$count = 0;
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_news WHERE status=? ORDER BY news_id DESC");
...
<div class="row">
	<div class="col-md-12">
		<div class="heading">
			<h2 class="sidebar-heading"> ቀዳሚ ዜናዎች</h2>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-sm-12 col-md-12">
		...
		$count = 0;
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_news WHERE status=? ORDER BY total_view DESC");
		$statement->execute(array('Published'));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach ($result as $row)
		{
	...
	<div class="col-md-12">
		<div class="newsletter-sidebar">
			...
			if(isset($_POST['form_subscribe']))
			{
			    ...
			    {
			        $valid = 0;
			        $error_message .= 'የኢሜይል አድራሻ ባዶ ሊሆን አይችልም';
			    }
			    else
			    {
				    ...
				    {
				        $valid = 0;
				        $error_message .= 'የኢሜል አድራሻ ትክክለኛ መሆን አለበት';
				    }
				    else
				    {
				    	$statement = $pdo->prepare("SELECT * FROM Maleda_News_subscriber WHERE subs_email=?");
				    	$statement->execute(array($_POST['email_subscribe']));
				    	$total = $statement->rowCount();							
				    	if($total)
				    	{
				    		$valid = 0;
				        	$error_message .= 'የኢሜል አድራሻው አስቀድሞ ተመዝግቧል';
				    	}
				    	else
				    	{
				    		...
				    		$statement = $pdo->prepare("INSERT INTO Maleda_News_subscriber (subs_email,subs_date,subs_date_time,subs_hash,subs_active) VALUES (?,?,?,?,?)");
				    		$statement->execute(array($_POST['email_subscribe'],$current_date,$current_date_time,$key,0));

				    		// Sending Confirmation Email
				    		$to = $_POST['email_subscribe'];
							$subject = 'የተመዝጋቢ ኢሜይል ማረጋገጫ';
...
ይህ አገናኝ ለ 24 ሰዓታት ብቻ ነው ገባሪ የሚሆነው።
ለመመዝገብ ፍላጎት በማሳየትዎ እናመሰግናለን!<br><br>
ምዝገባዎን ለማረጋገጥ እባክዎ ይህንን አገናኝ ጠቅ ያድርጉ:
...
ይህ አገናኝ ለ 24 ሰዓታት ብቻ ነው ገባሪ የሚሆነው።
					';
							...
							$success_message .= '<span style="color:#fff;">እባክዎ ኢሜይልዎን ይመልከቱ እና ምዝገባዎን ያረጋግጡ።</span>
							';
				    	}
				    }
			    }
			...
			<form action="<?php echo BASE_URL.$cur_page; ?>#subscribeForm" method="post">
				...
				<div><input type="submit" value="ይመዝገቡ" name="form_subscribe"></div>
			</form>
		</div>
	</div>
</div>
...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_advertisement WHERE adv_id=3");
$statement->execute();
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) 
{
