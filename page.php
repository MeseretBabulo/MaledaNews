...
else
{
	...
	$statement = $pdo->prepare("SELECT * FROM Maleda_News_page WHERE page_slug=? AND status=?");
...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_page WHERE page_slug=?");
...
if(isset($_POST['form_contact']))
{
    ...
    {
        $valid = 0;
        $error_message .= 'እባክዎ ስምዎን ያስገቡ.\n';
    }

    if(empty($_POST['visitor_phone']))
    {
        $valid = 0;
        $error_message .= 'እባክዎ ስልክ ቁጥርዎን ያስገቡ\n';
    }
    ...
    {
        $valid = 0;
        $error_message .= 'እባክዎ የኢሜል አድራሻዎን ያስገቡ\n';
    }
    else
    {
        ...
        {
            $valid = 0;
            $error_message .= 'እባክዎ ትክክለኛ ኢሜል ያስገቡ\n';
        }
    }
    ...
    {
        $valid = 0;
        $error_message .= 'እባክዎን አስተያየትዎን ያስገቡ\n';
    }
...
?>
				...
				<form action="" class="form-horizontal cform-1" method="post">
					<div class="form-group">
                        <div class="col-sm-12">
                            <input name="visitor_name" type="text" class="form-control" placeholder="ስም">
                        </div>
                    </div>
					<div class="form-group">
                        <div class="col-sm-12">
                            <input name="visitor_email" type="email" class="form-control" placeholder="
የ ኢሜል አድራሻ">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-12">
                            <input name="visitor_phone" type="text" class="form-control" placeholder="ስልክ ቁጥር">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-12">
                            <textarea name="visitor_comment" class="form-control" cols="30" rows="10" placeholder="መልእክት"></textarea>
                        </div>
                    </div>
                    <div class="form-group">
	                    <div class="col-sm-12">
	                        <input type="submit" value="መልዕክትዎን ይላኩ" class="btn btn-success" name="form_contact">
	                    </div>
	                </div>
				</form>
		...
		<div class="row">
			<div class="col-md-12">				
				...
				<section class="gallery">													
					<div class="item">
						<?php
						$statement = $pdo->prepare("SELECT * FROM Maleda_News_category_photo WHERE status=? ORDER BY p_category_id DESC");
						...
						foreach ($result as $row) 
						{
							...
							$statement1 = $pdo->prepare("SELECT * FROM Maleda_News_photo WHERE p_category_id=? ORDER BY photo_id DESC");
		...
		<div class="row">
			<div class="col-md-12">				
				...
				<section class="gallery">													
					<div class="item">
						<?php
						$statement = $pdo->prepare("SELECT * FROM Maleda_News_category_video WHERE status=? ORDER BY v_category_id DESC");
						$statement->execute(array('Active'));
						$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
						foreach ($result as $row) 
						{
							...
							$statement1 = $pdo->prepare("SELECT * FROM Maleda_News_video WHERE v_category_id=? ORDER BY video_id DESC");
