...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if($path == '') {
			...
			$statement = $pdo->prepare("UPDATE Maleda_News_file SET file_title=? WHERE file_id=?");
        	...
        	// updating into the database
			$statement = $pdo->prepare("UPDATE Maleda_News_file SET file_title=?, file_name=? WHERE file_id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM Maleda_News_file WHERE file_id=?");
