...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if($path == '') {
			...
			$statement = $pdo->prepare("UPDATE Maleda_News_photo SET photo_caption=?, p_category_id=? WHERE photo_id=?");
        	...
        	// updating into the database
			$statement = $pdo->prepare("UPDATE Maleda_News_photo SET photo_caption=?, photo_name=?, p_category_id=? WHERE photo_id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM Maleda_News_photo WHERE photo_id=?");
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select class="form-control" name="p_category_id">
									<?php
									$statement = $pdo->prepare("SELECT * FROM Maleda_News_category_photo ORDER BY p_category_name ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
									foreach ($result as $row) {
