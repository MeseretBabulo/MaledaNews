...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['page_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_page WHERE page_name=? and page_name!=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_page WHERE page_slug=? AND page_name!=?");
		...
		$statement = $pdo->prepare("UPDATE Maleda_News_page SET page_name=?, page_slug=?, page_content=?,page_layout=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE page_id=?");
