...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['p_category_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_category_photo WHERE p_category_name=? and p_category_name!=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE Maleda_News_category_photo SET p_category_name=?, status=? WHERE p_category_id=?");
