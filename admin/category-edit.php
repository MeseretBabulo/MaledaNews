...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['category_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_category WHERE category_name=? and category_name!=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_category WHERE category_slug=? AND category_name!=?");
		...
		$statement = $pdo->prepare("UPDATE Maleda_News_category SET category_name=?, category_slug=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE category_id=?");
