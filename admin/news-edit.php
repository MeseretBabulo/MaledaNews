...
if(isset($_POST['form1'])) {
	...
	if(empty($_POST['news_title'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_news WHERE news_title=? and news_title!=?");
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_news WHERE news_slug=? AND news_title!=?");
		...
		// Update Maleda_News_news_category making access value to 0 for particular news
		$statement = $pdo->prepare("UPDATE Maleda_News_news_category SET access=0 WHERE news_id=?");
		$statement->execute(array($_REQUEST['id']));

		// Update Maleda_News_news_category putting updated access value for particular news
		for($i=0;$i<count($arr1);$i++) {
			$statement = $pdo->prepare("UPDATE Maleda_News_news_category SET access=1 WHERE news_id=? AND category_id=?");
			$statement->execute(array($_REQUEST['id'],$arr1[$i]));
		}

		// Check Maleda_News_news_scheduled

		// If previous image not found and user do not want to change the photo
	    if($previous_photo == '' && $path == '') {
	    	$statement = $pdo->prepare("UPDATE Maleda_News_news SET news_title=?, news_slug=?, news_content=?, news_date=?, publisher=?,  status=?, source=?, is_featured=?, meta_title=?, meta_keyword=?, meta_description=? WHERE news_id=?");
	    ...
	    if($previous_photo == '' && $path != '') {
	    	...
	    	$statement = $pdo->prepare("UPDATE Maleda_News_news SET news_title=?, news_slug=?, news_content=?, news_date=?, publisher=?, photo=?, status=?, source=?, is_featured=?, meta_title=?, meta_keyword=?, meta_description=? WHERE news_id=?");
...
$statement = $pdo->prepare("SELECT * FROM Maleda_News_news_category WHERE news_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
	if($row['access'] == 1) {
				...
				<div class="box box-info">
					<div class="box-body">
				        ...
				        <div class="form-group">
							...
							<p style="margin-bottom: 6220px">   .                                                                                                                                                                                                                                       </p>
						<h3 class="seo-info"></h3>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label"> </label>
							<div class="col-sm-9">
