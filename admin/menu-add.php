...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("INSERT INTO Maleda_News_menu (menu_type,menu_name,category_or_page_slug,menu_order,menu_parent,menu_url) VALUES (?,?,?,?,?,?)");
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<select class="form-control" name="menu_name_and_category_or_page_slug">
												...
												<?php
												$statement = $pdo->prepare("SELECT * FROM Maleda_News_category ORDER BY category_name ASC");
												$statement->execute();
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<select class="form-control" name="menu_name_and_category_or_page_slug">
												...
												<?php
												$statement = $pdo->prepare("SELECT * FROM Maleda_News_page ORDER BY page_name ASC");
												$statement->execute();
												$result = $statement->fetchAll(PDO::FETCH_ASSOC);		
												foreach ($result as $row) {
