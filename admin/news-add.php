...
if(isset($_POST['form1'])) {
	...
	if(empty($_POST['news_title'])) {
		...
		// Duplicate Checking
    	$statement = $pdo->prepare("SELECT * FROM Maleda_News_news WHERE news_title=?");
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'Maleda_News_news'");
		$statement->execute();
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_news WHERE news_slug=?");
		...
		if($path=='') {
			...
			$statement = $pdo->prepare("INSERT INTO Maleda_News_news (news_title,news_slug,news_content,news_date,publisher,photo,status,source,is_featured,total_view) VALUES (?,?,?,?,?,?,?,?,?,?)");
			$statement->execute(array($_POST['news_title'],$news_slug,$_POST['news_content'],$_POST['news_date'],$publisher,'',$_POST['status'],$_POST['source'],$_POST['is_featured'],0));
		} else {
            ...
            $statement = $pdo->prepare("INSERT INTO Maleda_News_news (news_title,news_slug,news_content,news_date,publisher,photo,status,source,is_featured,total_view) VALUES (?,?,?,?,?,?,?,?,?,?)");
			$statement->execute(array($_POST['news_title'],$news_slug,$_POST['news_content'],$_POST['news_date'],$publisher,$final_name,$_POST['status'],$_POST['source'],$_POST['is_featured'],0));
		}

		// Getting all the categories from Maleda_News_category and putting all those into a new table Maleda_News_news_category for this newly added news
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_category ORDER BY category_id ASC");
		$statement->execute();
		...
		foreach ($result as $row) {
			...
			$statement1 = $pdo->prepare("INSERT INTO Maleda_News_news_category (news_id,category_id,access) VALUES (?,?,?)");
			$statement1->execute(array($ai_id,$row['category_id'],$access));
		}

		// Entry data into a new table Maleda_News_news_scheduled if news date is set as future date.
		$today = date('d-m-Y');
		if($_POST['news_date'] != $today) {
			$statement = $pdo->prepare("INSERT INTO Maleda_News_news_scheduled (news_id,news_date) VALUES (?,?)");
					...
					<div class="box-body">
						...
						<div class="form-group">
				            ...
				            <div class="col-sm-9">
				            	<?php
				            	$i=0;
				            	$statement = $pdo->prepare("SELECT * FROM Maleda_News_category WHERE status=? ORDER BY category_name ASC");
						...
						<div class="form-group">
				            <label for="" class="col-sm-2 control-label">Is Breaking News? <span>*</span></label>
				            <div class="col-sm-6">
				                <label class="radio-inline">
