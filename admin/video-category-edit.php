...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['v_category_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM Maleda_News_category_video WHERE v_category_name=? and v_category_name!=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE Maleda_News_category_video SET v_category_name=?, status=? WHERE v_category_id=?");
