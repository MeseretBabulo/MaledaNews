...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['category_name'])) {
    	...
    	$statement = $pdo->prepare("SELECT * FROM Maleda_News_category WHERE category_name=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'Maleda_News_category'");
		$statement->execute();
		...
		// Saving data into the main table Maleda_News_category
		$statement = $pdo->prepare("INSERT INTO Maleda_News_category (category_name,category_slug,status,meta_title,meta_keyword,meta_description) VALUES (?,?,?,?,?,?)");
		...
		// Getting all the news ids from Maleda_News_news and insert into Maleda_News_news_category
		...
		// Insert data into Maleda_News_home_category
		$statement = $pdo->prepare("INSERT INTO Maleda_News_home_category (category_id,category_order,category_layout) VALUES (?,?,?)");
