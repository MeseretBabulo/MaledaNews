...
if (isset($_POST['form1'])) {
    ...
    if(empty($_POST['email'])) {
    	...
    	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
	    	...
	    	$statement = $pdo->prepare("SELECT * FROM Maleda_News_user WHERE email=?");
    ...
    if($valid == 1) {
    	...
    	if($path=='') {
    		...
    		$statement = $pdo->prepare("INSERT INTO Maleda_News_user (full_name,email,phone,password,photo,role,status) VALUES (?,?,?,?,?,?,?)");
    		...
    		$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'Maleda_News_user'");
    		$statement->execute();
