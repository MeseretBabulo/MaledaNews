...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE Maleda_News_video SET video_title=?, video_iframe=?, v_category_id=? WHERE video_id=?");
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select class="form-control" name="v_category_id">
									<?php
									$statement = $pdo->prepare("SELECT * FROM Maleda_News_category_video ORDER BY v_category_name ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
									foreach ($result as $row) {
